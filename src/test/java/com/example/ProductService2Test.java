...
import com.example.service.impl.ProductService2;
...
public class ProductService2Test {
    ...
    void testAddProductDuplicate() throws Exception {
        ...
        Exception ex = assertThrows(Exception.class, () -> {
            service.addProduct(new Product(1, "Mobile", 20000));
        });
    ...
    void testGetProductSuccess() throws Exception {
        Product p = new Product(2, "Mobile", 20000);
        service.addProduct(p);
        Product fetched = service.getProduct(2);
        assertEquals("Mobile", fetched.getName());
        assertEquals(20000, fetched.getPrice());
    }
    ...
    void testUpdateProductSuccess() throws Exception {
        Product p = new Product(3, "Headphones", 15000);
        service.addProduct(p);
        service.updateProduct(3, "Wireless Headphone", 17000);
        Product updated = service.getProduct(3);
        assertEquals("Wireless Headphone", updated.getName());
        assertEquals(17000, updated.getPrice());
    }
    ...
    void testSortByPrice() throws Exception {
        ...
        service.addProduct(new Product(21, "Mobile", 20000));
        service.addProduct(new Product(22, "Headphones", 15000));
        service.sortByPrice();
        var list = new java.util.ArrayList<>(service.productMap().values());
        list.sort(java.util.Comparator.comparingDouble(Product::getPrice));
        assertEquals("Headphones", list.get(0).getName());
        assertEquals("Mobile", list.get(1).getName());
